package com.yangbingdong.redisoperv2.core.metadata.index;
...
public class EntityUnionIndex<T> implements EntityIndex<T> {
	...
	private StringBuilder resolveStringBuilder() {
		return primary ? new StringBuilder(entityName) : new StringBuilder(entityName).append(EntityIndex.getIdxKeyPrefix(unique));
	}
